...
**Date:** 2026-04-11
**Status:** Implemented. Key deviation: human-readable expected format is `"2.0KiB"` (no space, 1 decimal per `format_size` at `src/aggregate.rs:284`), not `"2 KiB"` as originally spec'd. See `tests/e2e_display.rs` for final state.
**Builds on:**
