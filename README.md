...
> Assessor: Claude (Anthropic) — model Claude Fable 5.1 (`claude-fable-5-1`), Effort: 25 (the session's reasoning-effort setting).
>
> Assessment date: 2026-09-13.
>
> Assessed version: 1.3.2 at commit `0ab311b` (two commits after the `v1.3.2` tag `2c6d215`; the two intervening commits touch only the README metrics table and add `.cargo/min-publish-age.toml`, no source changes).
>
> Method and scope of evidence: this is a from-scratch assessment. Every claim below was re-derived from the code on this pass; no earlier assessment of this or any other project — including the other AI assessments in this README — was used as input. All 38 Rust files under `src/` (14,661 lines including embedded tests) and `build.rs` were read in full. The 8 live-AWS suites in `tests/e2e_*.rs` (113 test annotations, gated behind `--cfg e2e_test`), the 2 ungated process-level suites `tests/cli_sigint_exit_code.rs` and `tests/cli_broken_pipe.rs` (6 tests), `tests/common/mod.rs`, `Cargo.toml`, `Cargo.lock` (via `cargo tree`), `deny.toml`, `.cargo/*.toml`, `CHANGELOG.md`, all five GitHub Actions workflows, and the coverage artifacts `lcov.info` / `llvm-cov-report.txt` were read as supporting evidence. The following were executed on the assessed tree (rustc 1.98.1, macOS): `cargo fmt --all --check` (clean), `cargo clippy --all-features` and `cargo clippy -- -D warnings` (clean), `cargo clippy --all-targets --all-features` (two test-only lints, see CI section), `cargo test --all-targets` (525 passed: 506 library, 13 binary, 6 integration; the e2e crates compile as empty without `--cfg e2e_test`), and `cargo deny -L error check` (advisories, bans, licenses, sources all ok). Two behaviours were also probed by running the built binary against an unreachable endpoint (`--aws-max-attempts 0`, and `RUST_LOG=aws_sigv4=trace` with `-v`). Line numbers cited are as of commit `0ab311b`.
>
> Limits of the evidence: this assessment cannot establish the absence of bugs. It includes no fuzzing, sanitizer or Miri runs, formal verification, or penetration testing. The live-AWS e2e suite was not run on this pass (it needs the maintainer's AWS profile); the coverage artifacts show its paths were executed during the run that produced them. Coverage measures what tests execute, not whether the executed code is correct.

**Question addressed.** s3ls is a read-only listing tool: it cannot modify or delete S3 data. The failure modes that matter are therefore (1) a **silently incomplete or duplicated listing** that automation treats as a full inventory, (2) **output injection** by attacker-named objects, (3) **credential leakage**, (4) **runaway API usage, concurrency, or memory**, and (5) **misleading exit codes**. Each is examined below against the code as it exists at v1.3.2.

#### Surface

The binary has no subcommands and no mutating S3 call anywhere in `src/`. The only S3 operations built are `ListObjectsV2` and `ListObjectVersions` (`src/storage/s3/mod.rs:94, 176`) and `ListBuckets` / `ListDirectoryBuckets` (`src/bucket_lister.rs:295, 356`). Mutating calls (`PutObject`, `DeleteObjects`, `CreateBucket`, …) exist only in the e2e fixture harness `tests/common/mod.rs`. Mode validation is two-sided and named: bucket-listing mode rejects 19 object-only flags and object mode rejects the 3 bucket-only flags with explicit error messages instead of silently ignoring them (`src/config/args/mod.rs:710-787`); sort fields are validated per mode, de-duplicated, and capped at two (`:694-700, 800-828`). Validation failures exit 2 via clap's error path with a trailing newline (`src/bin/s3ls/main.rs:61-74`, the v1.3.1 fix). Every one of these rejections has a unit test in `src/config/args/tests.rs`.

#### Listing completeness (the highest-stakes property)

- **Pagination anti-loop guards.** All six pagination loops — sequential objects and versions, parallel-discovery objects and versions, `ListBuckets`, `ListDirectoryBuckets` — fail loudly when the endpoint reports a truncated page without a forward token or marker, or returns the same token/marker twice (`src/storage/s3/mod.rs:503-549, 635-679`; `src/bucket_lister.rs:332-345, 383-396`). A buggy or hostile S3-compatible endpoint can truncate a listing but cannot make s3ls loop forever, and cannot make it silently skip a page the endpoint said existed. Each guard has a dedicated unit test for both the sequential and parallel engines (`mod.rs:1502-1677, 2712-2919`).
- **Parallel engine.** Recursive listings fan out over delimiter-discovered common prefixes, which partition the keyspace, so a given object is returned by exactly one prefix scan and parallelism cannot duplicate rows. The concurrency permit is held across leaf sequential scans and released by each parent before it awaits its children, so the semaphore cannot deadlock and the `--max-parallel-listings` bound holds at the leaves (`mod.rs:580-595, 686`); a regression test measures peak in-flight requests against a mock fetcher (`mod.rs:2237-2304`). A sub-task error or panic cancels the whole pipeline and surfaces as exit 1 rather than a truncated exit 0 (`mod.rs:738-750`; tests at `:1800-1861, 3055-3083`).
- **Pipeline shutdown discipline.** The lister drops its intermediate receiver before joining the storage task, so a cancelled run cannot deadlock against a full bounded queue (`src/lister.rs:70-74`, pinned by a timeout-guarded regression test at `:160-222`). Every spawned stage — storage, lister, aggregator, display writer — has its `JoinError` caught and converted into cancellation plus a reported error (`src/pipeline.rs:68-98`; `lister.rs:76-86`).
- **Filters fail closed.** `fancy-regex` matching returns `Result`; a runtime error (for example backtrack-limit exhaustion) cancels the pipeline and exits 1 rather than silently keeping or dropping the row (`src/filters/include_regex.rs:30-48`, `exclude_regex.rs:30-48`, `src/lister.rs:62-66`), pinned for both regex filters with a forced `RuntimeError`. Boundary semantics (`>=` for `--filter-larger-size` / `--filter-mtime-after`, strict `<` for their counterparts, `None` storage class treated as `STANDARD`, delete markers exempt from size and storage-class filters but subject to regex and mtime filters, common prefixes exempt from everything) are pinned by unit tests including `u64::MAX` edges and re-verified against live S3 with read-back `LastModified` pivots (`tests/e2e_filters.rs`, `e2e_filters_versioned.rs`).
- **Scale evidence.** The live suite includes a 16,082-object, 6–7-level fixture verified for exact set-completeness under four configurations — full recursive, prefix-scoped, `--max-depth 3` with the expected boundary prefixes, and `--max-parallel-listing-max-depth 1` (`tests/e2e_large_listing.rs`) — plus 1,000-object `--max-keys 10` runs in both objects and versions mode (`tests/e2e_listing.rs:636-732`) and versioned pagination with delete markers across page boundaries in both engines (`e2e_filters_versioned.rs:427-604`).
- **Sorting.** Sorting is stable (`sort_by` / rayon `par_sort_by`), multi-column via `Ordering::then_with`, with a test proving sequential and parallel sorts produce identical output and a live test proving the auto-appended secondary date sort for versioned listings (`src/aggregate.rs:144-169`; `tests/e2e_sort.rs:352-431`).

#### Output integrity (injection defense)

Object keys are attacker-controlled even on a trusted endpoint. By default, text-mode output replaces `\x00`–`\x1f` and `\x7f` with `\xNN` escapes in every string an attacker can influence on Amazon S3 — keys, common prefixes, owner display name and ID, bucket names — before padding or joining columns (`src/display/mod.rs:78-108`; `src/display/columns.rs`; `src/bucket_lister.rs:27-33`). The escape function is UTF-8-correct: the byte fast path is sound because UTF-8 continuation bytes are all `>= 0x80`, and the slow path iterates by `char`. A unit test pins the exact phantom-row attack (a key embedding `\n` plus a fake TSV row) being neutralized (`src/display/tsv.rs:273-296`), and the live suite uploads keys containing literal tabs and newlines and 2–4-byte UTF-8 (emoji, CJK Extension B) and asserts both escaped text output and byte-exact JSON round-trips (`tests/e2e_edge_cases.rs`). JSON mode always emits through `serde_json`, so control characters are safely escaped and keys are byte-preserved; `--raw-output` is an explicit opt-out that conflicts with `--json`. Two scoping notes: ETag, storage class, checksum fields, version IDs, `BucketRegion`, and `BucketArn` are not escaped in text mode — on Amazon S3 these are server-generated and cannot carry control bytes, but a hostile S3-compatible endpoint could inject through them (outside the documented security model). And the aligned formatter pads by `char` count, not display width, so East Asian wide characters visually drift columns; values wider than their column are never truncated, they shift the row right. Both are cosmetic.

#### Credential handling

`AccessKeys` derives `Zeroize` + `ZeroizeOnDrop`; its `Debug` masks the access-key ID to first-4/last-4 (fully redacted under 8 characters) and fully redacts the secret and session token, all pinned by unit tests (`src/types/mod.rs:45-90, 309-323`). The three secret-bearing arguments set `hide_env_values` (`src/config/args/mod.rs:446-454`), and a forked-process test proves `--help` prints the env var *names* but never their *values*, with a deliberate control showing the non-secret `TARGET_REGION` value does appear so the assertion would catch a regression (`src/config/args/tests.rs:1967-2006`). The `-vvv` config trace passes through the redacting `Debug` chain (`src/bin/s3ls/main.rs:37`). `--target-no-sign-request` disables credential loading entirely (`src/storage/s3/client_builder.rs:120-123`). A probe on this pass confirmed that exporting `RUST_LOG=aws_sigv4=trace` with `-v` did not print the secret key. Residual, inherent exposure: secrets passed on the command line are visible to local process inspection; `CLIArgs` holds them as plain `String`s that derive `Debug` and are dropped without zeroization after conversion (no production code prints `CLIArgs`, so this is a latent footgun rather than a leak); and the AWS SDK keeps its own non-zeroizing copies. `http://` custom endpoints are accepted (`src/config/args/value_parser/url.rs`) and remove transport confidentiality — an operator-controlled insecure mode.

#### Robustness against non-conforming endpoints

There are **no `unsafe` blocks in production code**; the only four `unsafe` sites are test-only environment-variable mutations under `rusty_fork` (`src/bin/s3ls/tracing_init.rs:99, 133`; `src/config/args/tests.rs:1974, 2012`). The production panic surface is eleven `expect`/`unwrap` sites, each infallible by construction: two semaphore acquisitions on a semaphore that is never closed (`src/storage/s3/mod.rs:427, 724`), two `pop` calls on a column vector that always holds at least three entries (`src/bucket_lister.rs:95, 154`), five `serde_json::to_string` calls on string-keyed maps (`bucket_lister.rs:272`; `src/display/json.rs:24, 106, 146, 170`), and two re-parses of values already validated by clap's value parsers (`src/config/args/mod.rs:601`; `value_parser/human_bytes.rs:16`). No S3 response field is unwrapped: a row missing its key or `LastModified` is *silently dropped* (`convert_object`, `mod.rs:914-916`, likewise `:957-960, 1005-1007`), a missing `IsTruncated` is treated as end-of-listing (`:150, 242`), and a negative size clamps to zero (`:915`). This is the fail-safe rather than fail-loud choice: against a pathologically non-conforming endpoint it under-reports without any warning. Both release profiles set `panic = "abort"`, so any surviving panic terminates the process without unwinding. `--aws-max-attempts 0` was probed and behaves as a single attempt per the SDK's retry semantics rather than panicking.

#### Operational semantics

- **Exit codes.** Usage errors exit 2, listing errors exit 1, success (including an empty result) exits 0, and Ctrl+C exits 130 (`src/bin/s3ls/main.rs:21, 116-120`; `src/types/error.rs:19-25`). All are pinned: 0/1/2 by live-binary tests including empty bucket, no-matching-prefix, `NoSuchBucket`, bad credentials, and clap errors (`tests/e2e_listing.rs:232-456`), and 130 by the ungated `tests/cli_sigint_exit_code.rs`, which runs the real binary against an in-process fake S3 endpoint serving endless truncated pages, sends SIGINT mid-listing on the sequential, versioned, and parallel paths, and asserts a caught-signal exit of 130 with a quiet stderr (`:204-244`), plus a control run asserting exit 0 without the signal. This closes the v1.2.x gap where an interrupted run was indistinguishable from success. The Ctrl+C flag is stored before the token is cancelled, and a unit test pins that ordering (`src/bin/s3ls/ctrl_c_handler/mod.rs:33-34, 129-151`).
- **Broken pipe.** `BrokenPipe` from `| head` exits 0 silently on the object-listing, bucket-listing, and shell-completion paths (`main.rs:48-59, 91-95, 130-134`), and the stderr tracing writer swallows `BrokenPipe` too (`tracing_init.rs:15-30`). The completion path is pinned by the ungated `tests/cli_broken_pipe.rs` (five shells, read end closed before spawn); the two listing paths by shell-pipeline e2e tests (`tests/e2e_edge_cases.rs:704-787`).
- **Rate limiting** is exact: the schedule is property-tested for every accepted rate (`refill × 1000 == rate × interval` for 10–65,535 rps, `src/storage/s3/mod.rs:878-894, 2148-2163`), and the limiter's acquire is cancellation-aware (`:307-318`).
- **Memory.** Default sort mode buffers the entire result set; the three bounded channels between stages each default to 200,000 entries (`src/config/mod.rs:91`; `src/pipeline.rs:57-59`; `src/lister.rs:26`). `--no-sort` streams with near-constant memory and still applies filters (live-pinned, `tests/e2e_filters.rs:1070-1104`). For very large buckets, memory is bounded only if the operator opts into `--no-sort`.
- **Partial output on error.** When an S3 call fails mid-listing, the aggregator in sort mode discards everything it buffered and prints nothing (`src/aggregate.rs:78-87`), whereas `--no-sort` will already have streamed the rows received before the failure. Both exit 1; operators piping `--no-sort` output must gate on the exit code, not on the presence of rows.

#### Supply chain and CI

The resolved TLS stack is a single `rustls 0.23.43` with `aws-lc-rs 1.18.0` and OS trust anchors; the legacy `rustls 0.21` alias is deliberately disabled (`Cargo.toml:31-37`), `ring` is absent from the resolved graph, and `openssl-sys` is banned in `deny.toml`. `h2` resolves to 0.4.18, matching the v1.3.2 changelog's RUSTSEC-2026-0258 fix, and `cargo deny -L error check` is clean with `advisories.ignore = []`, unknown registries and git sources denied, and a permissive-license allowlist. It runs per push and PR (`ci.yml`) and daily at 01:34 UTC (`cargo-deny.yml`). `Cargo.lock` is committed; every release build and the crates.io publish use `--locked`; release artifacts get signed build-provenance attestations (`actions/attest-build-provenance@v4`) and publishing uses OIDC trusted publishing. A nightly-only `.cargo/min-publish-age.toml` sets a 7-day `global-min-publish-age` for dependency resolution. CI builds and tests seven targets (x86_64/aarch64 × Linux gnu/musl, Windows MSVC ×2, macOS arm64) with blocking fmt, clippy `-D warnings`, and cargo-deny jobs; the two ungated integration suites run there. Gaps worth naming: no CI job compiles the `--cfg e2e_test` corpus (113 live tests are type-checked and run only on the maintainer's machine); the release workflow builds on tag push without running tests or waiting for `ci.yml`; most actions are referenced by mutable major-version tags, and `rust-clippy.yml` uses the deprecated `actions-rs/toolchain` (SHA-pinned, `continue-on-error`); `ci.yml` grants `pull-requests: write` at the top level though no job needs it; and CI's clippy gate omits `--all-targets`, so two style-level lints in unit-test code (`format_in_format_args` in `src/display/aligned.rs`, `needless_range_loop` in `src/display/tsv.rs`) pass unnoticed. Production code is clippy-clean under `-D warnings` and `--all-features`.

#### Coverage measurement

`llvm-cov-report.txt` reports 97.55% region (297 of 12,106 missed), 96.39% function (28 of 776), and 98.33% line (140 of 8,376) coverage overall; `lcov.info` carries the same line and function totals in LCOV form across the same 35 instrumented files. The report is a combined unit + live-e2e run: `src/bin/s3ls/main.rs` reaches 100% function and 90.80% line coverage, which is only possible with the spawned binary instrumented. On the completeness-critical paths: `storage/s3/mod.rs` 97.84% regions / 98.60% lines, `lister.rs` 96.55%, `aggregate.rs` 99.08%, `pipeline.rs` 90.86%; the formatters `display/tsv.rs`, `aligned.rs`, `aligned_formatter.rs`, and `one_line_formatter.rs` sit at 100.00% regions, `display/json.rs` at 97.05%. The weakest files are `bucket_lister.rs` (91.60% regions, 82.22% functions), `bin/s3ls/tracing_init.rs` (82.50% regions), and `config/args/mod.rs` (76.47% functions). Branch coverage is not measured (the branch columns are empty), and the artifacts come from a single maintainer-machine run; CI neither produces nor gates on coverage. Coverage is structural: a covered line can still be incorrect.

#### Known limitations and findings of this assessment

This from-scratch pass found no critical or high-severity defects at v1.3.2. What it found, ordered by how likely an operator is to meet it:

- **Generic environment-variable names on every long option.** The positional target no longer reads `TARGET` (regression-tested, `src/config/args/tests.rs:2008-2025`), but every long option is env-backed under its bare name: an exported `RECURSIVE`, `JSON`, `TSV`, `SORT`, `REVERSE`, `HEADER`, `MAX_KEYS`, `SUMMARIZE`, `NO_SORT`, and so on silently changes output shape or API-call volume, and because clap applies `conflicts_with` to env-sourced values too, an exported `JSON=true` makes `--tsv` fail with exit 2. Command-line values win over the environment, and for a read-only tool the blast radius is output shape rather than data loss, but scripts running in CI environments where such names are plausible can be surprised.
- **Fail-safe, not fail-loud, on non-conforming responses.** Rows missing a key or timestamp are dropped without a warning, and a missing `IsTruncated` ends the listing as if complete. Correct for Amazon S3, where these fields are always present; against a broken S3-compatible endpoint the result is silent under-reporting. The anti-loop guards cover the inverse case (loud failure on impossible pagination states), so the asymmetry is deliberate but worth knowing.
- **Directory-marker objects at the `--max-depth` boundary render differently between the two engines.** `key_depth` strips one trailing `/` so a zero-byte marker like `p/sub/` counts at the same depth as `p/file.txt` (`src/storage/s3/mod.rs:367-381`). In the sequential engine (used with `--max-parallel-listings 1` or Express One Zone buckets) such a marker at exactly depth `N` is emitted as an object row *and* the objects beneath it collapse into a `PRE p/sub/` row; the parallel engine never fetches that level and emits only the `PRE` row (`:575-579, 691-708`), which matches S3's native delimiter semantics. Same bucket, same flags, one extra size-0 row depending on engine. Cosmetic, unpinned by any test.
- **Signals other than Ctrl+C.** SIGTERM is not handled (default disposition kills the process, which at least cannot read as success), and the bucket-listing path returns before the Ctrl+C handler is installed (`main.rs:87-104`), so Ctrl+C there terminates by raw signal rather than the graceful 130 path — the shell still reports 130. The 130 exit code is a Unix convention; the SIGINT tests are `cfg(unix)`, so Windows behaviour is unpinned.
...
- **Escaping is scoped to attacker-controllable-on-AWS fields**, not every S3-returned string (see Output integrity). Operators of untrusted endpoints should prefer `--json`.
- **Comment drift.** A number of `Covers src/...:NNN` references in the e2e comments point at pre-refactor locations (for example `src/aggregate.rs` for formatter code that now lives under `src/display/`). Cosmetic only.
- No fuzzing, sanitizers, Miri, or external audit; the e2e suite is maintainer-only. Fuzzing `escape_control_chars`, `S3Target::parse`, `format_key_display`, and the depth arithmetic (`key_depth` / `prefix_at_depth`) would be the highest-value next step.

The changelog fixes claimed since v1.2.0 were each verified present with a pinning test: the completion-script broken-pipe fix (v1.2.2, `tests/cli_broken_pipe.rs`), the SIGINT exit code 130 (v1.3.0, `tests/cli_sigint_exit_code.rs` and `src/bin/s3ls/main.rs:116-120`), the trailing newline on validation errors (v1.3.1, `main.rs:67-71`; present in code, no test asserts the newline itself), and the patched `h2` (v1.3.2, resolved 0.4.18 with a clean advisory check). The earlier fixes — leaf-scan concurrency cap, exact rate limiter, credential values hidden from `--help`, and positional `TARGET` env removal — remain pinned by `parallel_leaf_scans_respect_concurrency_limit`, `rate_limiter_schedule_is_exact_for_all_valid_rates`, `help_hides_credential_env_values_but_shows_names`, and `positional_target_ignores_target_env_var`.
...
**Risk: damage to your data.** Structurally excluded: the binary contains only the four S3 list operations. Nothing in the codebase can write, modify, or delete an object or bucket.

**Risk: silently incomplete listings.** The engine fails loudly on impossible pagination states, propagates every worker error and panic into a non-zero exit, bounds concurrency correctly, and its completeness is verified against live S3 at 16,000-object scale in four configurations. An interrupted run now exits 130, so a supervisor can tell "cancelled" from "complete". The remaining caveat is that a malformed response from a non-AWS endpoint is dropped quietly rather than reported.
...
**Risk: credential leakage.** Keys are zeroized and redacted in every debug, log, and help path, with regression tests for the one leak that previously existed (`--help` printing exported credential values, fixed in v1.1.0).
...
**What this assessment cannot establish.** That the code is bug-free, that S3-compatible stores other than Amazon S3 will behave identically (the fail-safe conversions assume AWS-shaped responses), or that future dependency advisories will not bite before they are patched. In plain terms: at v1.3.2 the failure modes that could cause silent harm each have a specific, citable, tested safeguard, and the tool is reliable for routine use *provided* the operator treats exit codes — not output presence — as the success signal and avoids generic environment variables that collide with s3ls option names. The fact that the codebase was generated by AI does not by itself raise or lower its reliability; what determines that is the design, the tests, and the verifiable evidence above.
...
**Evaluator:** OpenAI Codex; **model:** GPT-5 (the exact variant is not exposed in this session); **Effort:** not exposed in this session. Assessed the working tree at commit `3ea5653` on 2026-09-13.

This is an independent, from-scratch assessment of the whole application source: all 38 Rust files under `src/` and `build.rs`. I examined the object and bucket listing paths, configuration and credentials, filters, sorting, every output format, cancellation and error handling, and the embedded tests. I also checked the process-level and live-AWS integration test suites, `Cargo.toml`/`Cargo.lock`, `deny.toml`, CI workflows, and the supplied `lcov.info` and `llvm-cov-report.txt`. No other AI assessment was used as evidence.

#### Verification and evidence

On this tree, `cargo test --locked --offline --all-targets` passed 523 library tests, 13 binary tests, and 6 process-level integration tests. `cargo fmt --all --check`, `cargo clippy --locked --offline --all-features -- -D warnings`, and `cargo deny -L error check` passed. The eight `tests/e2e_*.rs` suites are gated by `cfg(e2e_test)` and ran **zero tests** in that local command; I did not run a live-AWS test or regenerate coverage.

The supplied coverage artifacts agree on 8,236/8,376 lines (98.33%) and 748/776 functions (96.39%); `llvm-cov-report.txt` also records 11,809/12,106 regions (97.55%). The report measures zero branches. It shows high coverage in `storage/s3/mod.rs` (98.60% lines), `aggregate.rs` (99.07%), and the text formatters, but weaker coverage in `bucket_lister.rs` (82.22% functions) and `pipeline.rs` (92.67% lines). These are pre-existing artifacts, not results of this review's test run, and coverage alone does not establish that a listing is complete.

#### Assessment

The production S3 calls are read-only listing operations: `ListObjectsV2`, `ListObjectVersions`, `ListBuckets`, and `ListDirectoryBuckets`. Mutating calls appear in the live-AWS test fixture, not the CLI listing path. The AWS SDK handles transport, signing, credentials, retries, and optional timeouts. Explicit access keys are masked in the application's `Debug` output, and the no-sign option uses the SDK's no-credentials mode. Text output escapes ASCII control characters in S3-supplied names by default; JSON serialization preserves valid NDJSON structure. `--raw-output` and an explicitly configured `http://` endpoint intentionally relax those protections.

The listing pipeline uses bounded channels and a semaphore for concurrent requests. It propagates SDK, filter, worker, and output errors; guards against missing or immediately repeated pagination markers; and checks cancellation during listing and output. Range splitting has dedicated tests for flat, skewed, Unicode, versioned, and nonconforming-endpoint cases. `--no-sort` streams object entries, while the default sort collects them all before output.

#### Findings and limits

- **Custom AWS file flags require an explicit profile:** `--aws-config-file` and `--aws-shared-credentials-file` are recorded in the client config, but `build_profile_files()` is used only for `S3Credentials::Profile` in `src/storage/s3/client_builder.rs`. With default environment credentials, those flag values do not replace the SDK's ambient file locations. A user who supplies only a custom file path can authenticate with unintended ambient credentials or fail to authenticate; pair the file flags with `--target-profile` or use the SDK's standard environment configuration.
- **Silent omission on malformed object metadata:** `convert_object`, `convert_object_version`, and `convert_delete_marker` return `None` when the key or last-modified timestamp is absent or invalid, and the page fetcher drops those entries with `filter_map` or equivalent loops (`src/storage/s3/mod.rs`). The command can therefore exit successfully with an incomplete result from a malformed S3-compatible response. This is less likely against conforming AWS S3 responses, but matters when completeness is the reason for using the tool.
- **Pagination guards cover only immediate repetition:** object, version, and bucket loops reject a marker equal to the previous marker. A nonconforming endpoint that cycles through two or more markers can keep a listing running and issuing requests. The default object-listing rate is unrestricted unless `--rate-limit-api` is supplied.
- **Memory and consistency are operational limits:** sorted object listing retains every entry in memory; bucket listing retains every bucket even with `--no-sort`. Streaming object mode uses bounded queues, but their default capacity is 200,000 entries per channel, so memory use can still be substantial. A listing is not a point-in-time snapshot, so concurrent bucket mutations can change what pagination returns. The code cannot promise an audit-complete inventory under mutation.
- **Credential and output safeguards have boundaries:** `AccessKeys` zeroizes its own strings on drop, but CLI argument strings, clones, and SDK-held credential copies are outside that guarantee. The text escape routine handles ASCII C0/DEL controls; it does not neutralize every Unicode formatting character. Users can also select raw output or plaintext HTTP for a custom endpoint.
- **Test evidence is strong but not exhaustive:** CI runs normal cargo tests and static checks, while the live-AWS suites require an explicit cfg and credentials. The supplied coverage report has no branch data, and this pass did not independently reproduce its live-service coverage. Custom endpoint behavior, error-path combinations, and large mutable listings remain the main evidence gaps.

**Reliability judgment:** This is a well-tested, read-only listing tool with sensible defaults for ordinary inspection. I would not treat a successful exit as proof of complete inventory on a nonconforming endpoint or a changing bucket. For a completeness-sensitive workflow, use independent counts or S3 Inventory and account for the findings above.
...
> Assessment date: 2026-09-13.
>
> Evaluator: Gemini (Model: Gemini 3.8 Flash, Effort: High).
>
> Assessed version: 1.3.2 (commit `dd50958`).
>
> Method and scope of evidence: this assessment was performed from scratch at v1.3.2, deriving all claims directly from whole-source inspection and test execution without reference to other AI assessments. All 38 Rust source files under `src/` (14,661 lines including embedded unit tests) and `build.rs` (8 lines) were read and verified in full. In addition, all 8 live-AWS integration suites under `tests/e2e_*.rs`, the process-level test suites `tests/cli_sigint_exit_code.rs` and `tests/cli_broken_pipe.rs`, `tests/common/mod.rs` (9,096 lines across `tests/`), `Cargo.toml`, `Cargo.lock`, `deny.toml`, all GitHub Actions workflows, and the latest coverage artifacts (`lcov.info` and `llvm-cov-report.txt`) were audited as supporting evidence. Verification commands executed on the tree (rustc 1.98.1, macOS): `cargo fmt --all --check` (clean, 0 diffs), `cargo clippy --all-features -- -D warnings` (clean, 0 warnings), `cargo clippy --all-targets --all-features` (2 minor style lints in test modules), `cargo test --all-targets` (525 passed: 506 library unit tests, 13 binary unit tests, 4 `cli_sigint_exit_code`, 2 `cli_broken_pipe`; e2e integration targets compile as empty without `--cfg e2e_test`), and `cargo deny check` (advisories, bans, licenses, sources all ok). Line numbers cited reflect commit `dd50958`.
>
> Limits of the evidence: this assessment cannot rule out all potential software defects. It does not incorporate kernel-level fault injection, formal verification, or fuzzing runs. Live-AWS e2e tests execute against maintainer AWS infrastructure; verified coverage artifacts document execution across these paths during generating test runs. Coverage metrics measure statement and branch execution, not logical completeness.

**Question addressed.** s3ls is a read-only listing tool — it contains no mutating S3 operations and cannot create, modify, overwrite, or delete data in S3. The critical failure modes evaluated are: (1) a **silently incomplete or duplicated listing** treated as complete inventory by downstream automation, (2) **output injection** (maliciously-named keys injecting rows or ANSI terminal escapes into operator output), (3) **credential leakage**, (4) **runaway API calls or memory exhaustion**, and (5) **misleading exit codes**.

#### Surface

The binary contains no mutating operations anywhere in `src/`: the only S3 API calls in production code are `ListObjectsV2` (`src/storage/s3/mod.rs:94`), `ListObjectVersions` (`src/storage/s3/mod.rs:176`), `ListBuckets` (`src/bucket_lister.rs:295`), and `ListDirectoryBuckets` (`src/bucket_lister.rs:356`). Mutating operations (`PutObject`, `DeleteObject`, `CreateBucket`, etc.) are confined exclusively to test fixtures in `tests/common/mod.rs`. CLI mode validation strictly separates bucket-listing mode and object-listing mode (`src/config/args/mod.rs:710-787`): object-only flags in bucket mode fail with exit code 2, and bucket-only flags in object mode fail with exit code 2. Validation error messages terminate with a newline (`src/bin/s3ls/main.rs:69`), preventing prompt mangling. Positional target parsing does not read ambient `TARGET` environment variables (`src/config/args/mod.rs:90-97`, `tests.rs:2009-2025`). Usage errors exit 2, runtime listing errors exit 1, user cancellation exits 130, and successful runs exit 0.

#### Listing completeness (the highest-stakes property)

- **Pagination anti-loop guards.** All six pagination loops — sequential objects/versions, parallel-discovery objects/versions, `ListBuckets`, and `ListDirectoryBuckets` — fail loudly with an error if S3 returns a truncated response without a continuation token/marker, or returns the identical token or marker pair twice (`src/storage/s3/mod.rs:508-524, 529-548, 640-655, 660-679`; `src/bucket_lister.rs:338-345, 388-395`). A non-conforming or malfunctioning S3 endpoint can terminate listing with an error, but cannot induce infinite looping or silent page loss. Dedicated tests verify each guard (`src/storage/s3/mod.rs:1502-1677, 2712-2919`).
- **Parallel engine correctness.** Recursive listings partition key namespaces by delimiter-discovered common prefixes, guaranteeing that each object is enumerated by exactly one prefix scan and preventing duplicate rows. Concurrency permits (`listing_worker_semaphore`) are held across leaf sequential listings (`depth > max_parallel_listing_max_depth`), properly enforcing `--max-parallel-listings` on deep prefixes (`src/storage/s3/mod.rs:589-595`). Permits are released before awaiting sub-task joins (`src/storage/s3/mod.rs:686`), preventing semaphore deadlocks. Sub-task panics or runtime errors cancel the pipeline via `CancellationToken` and propagate an error exit 1 rather than partial success (`src/storage/s3/mod.rs:738-750`).
- **Pipeline shutdown discipline.** The lister stage explicitly drops its intermediate receiver channel (`list_rx`) before awaiting the storage task handle (`src/lister.rs:74`). This ensures a cancelled pipeline cannot deadlock against a full bounded channel (200,000 entries) when storage workers attempt to send (`src/lister.rs:70-74, 160-222`). Pipeline stages catch `JoinError` and convert panics into pipeline cancellation and structured error reports (`src/pipeline.rs:68-98`).
- **Filters fail closed.** Synchronous `FilterChain` evaluation treats regex matching errors (such as backtrack-limit exhaustion in `fancy-regex`) as fatal, cancelling the pipeline and exiting 1 instead of silently dropping or retaining entries (`src/filters/include_regex.rs:34-48`, `exclude_regex.rs:34-48`; `src/lister.rs:62-66`). Size and mtime boundary semantics (`>=` for `--filter-larger-size` and `--filter-mtime-after`, strict `<` for smaller/before filters, `None` storage class treated as `STANDARD`, delete markers exempt from size filters) are unit-tested and verified against live S3 (`tests/e2e_filters.rs`, `e2e_filters_versioned.rs`).
- **Scale evidence.** Live integration suites assert set completeness against a 16,082-object, 6–7 level deep fixture across four configurations (full recursive, prefix-scoped, `--max-depth 3`, `--max-parallel-listing-max-depth 1`), alongside 1,000-object paginated runs (`--max-keys 10`) in both object and version modes (`tests/e2e_large_listing.rs`, `e2e_listing.rs:636-732`).
- **Sorting.** Sorting is stable (`sort_by` / rayon `par_sort_by`), multi-column via `Ordering::then_with` (`src/aggregate.rs:144-168`), with tests proving identical output between sequential and parallel sorts. When `--all-versions` is active with a single sort field, `Date` is automatically appended as a secondary sort to keep object versions chronologically ordered (`src/config/args/mod.rs:834-836`; `tests/e2e_sort.rs:352-431`).

#### Output integrity (injection defense)

In default text output modes (aligned, TSV, one-line), control characters (`\x00`–`\x1f` and `\x7f`) in attacker-controlled strings — object keys, common prefixes, owner names/IDs, bucket names — are sanitized into `\xNN` hex escapes before column alignment or formatting (`src/display/mod.rs:78-99`; `src/display/columns.rs`; `src/bucket_lister.rs:27-33`). The byte fast-path is sound for UTF-8 (continuation bytes are `>= 0x80`; slow path iterates by `char`), preserving multi-byte UTF-8 sequences without corruption. Unit tests confirm neutralization of phantom-row injection attacks (embedding `\n` and fake TSV fields in object keys) (`src/display/tsv.rs:273-296`). Live AWS tests upload keys with tabs, newlines, and 2–4-byte UTF-8 characters (emoji, CJK) asserting correct escaping and byte-exact JSON serialization (`tests/e2e_edge_cases.rs`). JSON mode emits via `serde_json`, ensuring standard control-character escaping. `--raw-output` explicitly opts out of escaping and conflicts with `--json`. Note: system metadata fields (`BucketRegion`, `BucketArn`, ETag, Version ID) are not passed through text escape routines; while Amazon S3 guarantees these contain no control bytes, non-AWS S3 endpoints could theoretically deliver unescaped bytes through them.

#### Credential handling

`AccessKeys` derives `Zeroize` and `ZeroizeOnDrop` (`src/types/mod.rs:45-50`). Its `Debug` implementation masks access key IDs to first 4 and last 4 characters (fully redacting IDs `< 8` chars) and completely redacts secret keys and session tokens (`src/types/mod.rs:67-90`). Secret-bearing CLI arguments enable `hide_env_values`, and process-forked tests verify `--help` prints environment variable names while suppressing secret values (`src/config/args/tests.rs:1967-2006`). Configuration trace logs (`-vvv`) format credential structs via redacting `Debug` implementations (`src/bin/s3ls/main.rs:37`). `--target-no-sign-request` skips credential resolution for public bucket access (`src/storage/s3/client_builder.rs:120-123`). Secrets supplied directly as CLI arguments remain visible in process table listings (`ps`), and custom `http://` URLs disable transport encryption by operator choice.

#### Robustness against non-conforming endpoints

There are **zero `unsafe` blocks in production code** (the only 4 `unsafe` occurrences in `tracing_init.rs` and `args/tests.rs` are test-only environment variable mutations under `rusty_fork`). The production panic surface consists of 11 provably infallible `expect`/`unwrap` sites:
1. Four calls in `src/display/json.rs:24, 106, 146, 170` serializing valid JSON primitive maps via `serde_json::to_string`.
2. Three calls in `src/bucket_lister.rs:95, 154` (popping from pre-populated non-empty column vectors) and line 272 (`serde_json::to_string(&map)`).
3. Two calls in `src/storage/s3/mod.rs:427, 724` acquiring permits on unclosed Tokio semaphores.
4. One call in `src/config/args/value_parser/human_bytes.rs:16` converting pre-validated byte ranges to `u64`.
5. One call in `src/config/args/mod.rs:601` compiling regexes already validated by clap's value parser.

No S3 response field is unwrapped: malformed rows missing key or timestamp fields are safely dropped via `filter_map` (`convert_object`, `src/storage/s3/mod.rs:138, 914-916`), missing `IsTruncated` defaults to false, and negative object sizes clamp to 0. This fail-safe design avoids crashes on malformed responses, though non-conforming third-party endpoints may result in dropped rows rather than loud errors. Release compilation profiles enforce `panic = "abort"` (`Cargo.toml:72, 81`), ensuring any unexpected panic terminates the process without unwinding across FFI boundaries.

#### Operational semantics

- **Exit codes** (0, 1, 2, 130) are pinned by tests: empty bucket or empty prefix matches return exit 0; S3 access errors and `NoSuchBucket` return exit 1; clap validation errors return exit 2.
- **Ctrl-C (SIGINT) exits 130 (Resolved in v1.3.0).** Interruption by Ctrl-C triggers `ctrl_c_handler`, records an atomic flag, cancels the pipeline, and terminates the process with exit code **130** (`SIGINT_EXIT_CODE = 128 + 2`) (`src/bin/s3ls/main.rs:18-21, 116-120`; `src/bin/s3ls/ctrl_c_handler/mod.rs`). Process-level tests in `tests/cli_sigint_exit_code.rs` assert that automated scripts can distinguish user interruption from successful completion (exit 0) and runtime errors (exit 1).
- **Broken pipe resilience.** Writing listing output or shell completion scripts into a closed downstream pipe (e.g. `s3ls ... | head`) exits 0 silently without broken-pipe panics or noisy tracing errors (`src/bin/s3ls/main.rs:54, 91-95, 130-134`; `tests/cli_broken_pipe.rs`).
- **Rate limiting** is exact: `rate_limiter_schedule` computes integer token refills across exact millisecond intervals for all rates (`10..=65535`), avoiding rounding bugs and handling cancellation signals cleanly (`src/storage/s3/mod.rs:878-894, 2148-2182`).
- **Memory usage:** Default sort mode buffers listing results in memory prior to sorting, using bounded intermediate channels (200,000 entries per stage) (`src/config/mod.rs:91`; `src/pipeline.rs:57-59`; `src/lister.rs:26`). Streaming mode (`--no-sort`) operates in near-constant memory while retaining full filtering capabilities (`tests/e2e_filters.rs:1070-1104`).

#### Supply chain and CI

The shipping TLS stack relies on `rustls 0.23.42` with `aws-lc-rs 1.17.3` and system trust anchors; legacy `rustls 0.21` is explicitly excluded to prevent RUSTSEC-2026-0098 (`Cargo.toml:31-37`), and `openssl-sys` is banned in `deny.toml:37`. `h2` is updated to a patched release resolving RUSTSEC-2026-0258. `cargo deny check` runs on pull requests (`ci.yml`) and daily schedules (`cargo-deny.yml`) with zero ignored advisories. `Cargo.lock` is committed; release builds enforce `--locked` and emit OIDC build-provenance attestations (`actions/attest-build-provenance@v4`). CI tests 7 target architectures. Note: CI clippy checks omit `--all-targets`, leaving 2 style-level warnings in test modules (`src/display/aligned.rs:154`, `src/display/tsv.rs:539`). Production code is clippy-clean under `-D warnings`.

#### Coverage measurement

`llvm-cov-report.txt` (and `lcov.info`) from combined unit + live-AWS e2e runs reports 97.55% region coverage (297/12,106 missed), 96.39% function coverage (28/776 missed), and 98.33% line coverage (140/8,376 missed). Critical modules exhibit high coverage: `storage/s3/mod.rs` (97.84% region / 98.60% line), `lister.rs` (96.55% region / 96.53% line), `aggregate.rs` (99.08% region / 99.07% line), `display_writer.rs` (98.42% region / 100.00% line), and output formatters (`display/tsv.rs`, `aligned.rs`, `aligned_formatter.rs`, `one_line_formatter.rs`) at 100.00% region coverage. Weakest files are `bucket_lister.rs` (91.60% region, 82.22% function) and `tracing_init.rs` (82.50% region).

#### Known limitations and findings of this assessment

This from-scratch assessment identified no critical or high-severity security vulnerabilities at v1.3.2. Findings and design trade-offs confirmed:

- **SIGINT exits 130 (Resolved defect from v1.2.0).** Automation scripts can now reliably distinguish interrupted runs from successful completions. SIGTERM remains unhandled by an application signal handler, relying on default OS process termination (exit 143).
- **Generic environment variable bindings on CLI options.** All long flags bind to generic env var names (`RECURSIVE`, `JSON`, `SORT`, `MAX_KEYS`, `HEADER`, etc.), meaning ambient environment variables can alter output formats or API call counts unless explicitly overridden on the command line. Note: the positional target argument does not bind to `TARGET`.
- **Unescaped system metadata fields in text output.** System metadata fields (`BucketRegion`, `BucketArn`, ETag, Version ID) are not passed through text escape routines; while Amazon S3 guarantees these contain no control bytes, non-standard third-party endpoints could theoretically inject control bytes through them.
...
**Risk: silently incomplete listings.** Safeguarded against pagination loops, worker panics, and filter failures, with set completeness verified against live S3 at 16,000-object scale. Interrupted runs now exit with code 130 rather than 0, allowing automation to reject incomplete outputs.
...
**What this assessment cannot establish.** That the software is completely free of bugs, that non-AWS S3 endpoints behave identically in fail-safe edge cases, or that future dependencies will remain vulnerability-free. In conclusion, at v1.3.2 all major failure modes have tested, verifiable safeguards, and `s3ls` is reliable for production listing tasks provided operators manage generic environment variable risks and stream very large listings (`--no-sort`).
